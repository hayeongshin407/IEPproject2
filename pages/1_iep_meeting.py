...
+
+
+def queue_ai_refinement(input_key, content_key, content_type):
...
+        st.session_state[f"{input_key}_feedback"] = "보완 결과가 원문과 동일합니다. 입력 내용을 조금 더 구체적으로 작성해 보세요."
+    else:
+        st.session_state[f"{input_key}_feedback"] = "AI 보완이 완료되었습니다."
+    st.rerun()
+
+
+def render_ai_refinement_section(title, expander_label, content_key, input_key, button_key, content_type):
+    st.markdown(f"#### {title}")
...
+
+        if st.button("AI가 내용 보완하기", key=button_key):
+            queue_ai_refinement(input_key, content_key, content_type)
+
+        if feedback_key in st.session_state:
+            if st.session_state[feedback_key] == "AI 보완이 완료되었습니다.":
...
+
+        if 'other_opinion_input_pending' in st.session_state:
+            st.session_state.other_opinion_input = st.session_state['other_opinion_input_pending']
+            st.session_state.meeting_contents['기타 의견'] = st.session_state['other_opinion_input_pending']
+            del st.session_state['other_opinion_input_pending']
+
+        st.text_area(
+            "간단히 작성하면 AI가 보완해 줍니다.",
+            key="other_opinion_input"
...
+
+        if st.button("AI가 내용 보완하기", key="btn_other_ai"):
+            queue_ai_refinement("other_opinion_input", "기타 의견", "의견")
+
+        if "other_opinion_input_feedback" in st.session_state:
+            if st.session_state["other_opinion_input_feedback"] == "AI 보완이 완료되었습니다.":
+                st.success(st.session_state["other_opinion_input_feedback"])
+            else:
+                st.info(st.session_state["other_opinion_input_feedback"])
+    
+    st.markdown("---")
+
+    st.header("✅ 의결 사항")
+    with st.expander("의결 사항 작성", expanded=True):
+        if 'resolution_input' not in st.session_state:
+            st.session_state.resolution_input = st.session_state.meeting_contents['의결 사항']
+
+        if 'resolution_input_pending' in st.session_state:
+            st.session_state.resolution_input = st.session_state['resolution_input_pending']
+            st.session_state.meeting_contents['의결 사항'] = st.session_state['resolution_input_pending']
+            del st.session_state['resolution_input_pending']
+
+        st.text_area(
+            "간단히 작성하면 AI가 보완해 줍니다.",
+            key="resolution_input"
+        )
+        st.session_state.meeting_contents['의결 사항'] = st.session_state.resolution_input
+
+        if st.button("AI가 의결 사항 보완하기", key="btn_resolution_ai"):
+            queue_ai_refinement("resolution_input", "의결 사항", "의결 사항")
+
+        if "resolution_input_feedback" in st.session_state:
+            if st.session_state["resolution_input_feedback"] == "AI 보완이 완료되었습니다.":
+                st.success(st.session_state["resolution_input_feedback"])
+            else:
+                st.info(st.session_state["resolution_input_feedback"])
+
+st.markdown("---")
+st.subheader("📥 회의록 워드 파일 생성 및 다운로드")
+st.markdown("입력된 내용을 바탕으로 워드 문서 파일을 생성합니다.")
