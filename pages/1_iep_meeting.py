...
# ✅ [수정] API 키를 메인 앱의 session_state에서 가져와서 사용
if 'user_api_key' not in st.session_state:
    st.warning("메인 화면에서 Gemini API 키를 먼저 입력해주세요.")
    st.stop()
else:
...
with st.container(border=True):
    ...
    attendees = st.text_input("5. 참석자 (쉼표로 구분하여 작성)", key="meeting_attendees", value="홍길동(담임), 김철수(특수교사), 이영희(보호자)")
...
if st.button("📄 워드 파일(.docx) 생성 및 다운로드"):
    if not st.session_state.meeting_contents['의결 사항']:
        ...
        with st.spinner("회의록 Word 문서를 생성 중입니다..."):
            ...
            title = document.add_heading('개별화교육지원팀 협의회 회의록', level=0); title.alignment = WD_ALIGN_PARAGRAPH.CENTER
            ...
            table = document.add_table(rows=4, cols=2); table.style = 'Table Grid'
            ...
            for section, content in st.session_state.meeting_contents.items():
                if content and section != '의결 사항':
                    section_title = f"- {st.session_state.other_opinion_author} 의견: " if section == '기타 의견' and st.session_state.other_opinion_author else f"- {section}: "
